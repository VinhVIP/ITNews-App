...
class FollowersPage extends StatelessWidget {
  const FollowersPage({Key? key, required this.author}) : super(key: key);
  final User author;
  ...
  Widget build(BuildContext context) {
    return FractionallySizedBox(
      ...
      child: Column(
        children: [
          const ModalHeader(title: 'Followers'),
          BlocProvider(
            create: (_) =>
                AuthorsBloc(AccountRepository(httpClient: http.Client()))
                  ..add(AuthorFollowersFetched(author.idAccount)),
            child: const FolllowersList(),
          ),
        ],
      ),
    );
...
class FolllowersList extends StatelessWidget {
  const FolllowersList({Key? key}) : super(key: key);
